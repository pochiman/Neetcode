...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int: # type: ignore
        ...
        heapq.heapify(maxHeap) # type: ignore

        time = 0
        q = deque()  # type: ignore # pairs of [-cnt, idleTime]
        while maxHeap or q:
            time += 1
            if maxHeap:
                cnt = 1 + heapq.heappop(maxHeap) # type: ignore
                if cnt:
                    q.append([cnt, time + n])
            if q and q[0][1] == time:
                heapq.heappush(maxHeap, q.popleft()[0]) # type: ignore
        return time